...
//    private Stream<Token> tokenize(BufferedReader file) throws IOException {
//        Stream.Builder<Token> tokens = Stream.builder(); Char c;
//        String line; int row = 0;
//        while((line = file.readLine()) != null) { row++;
//            for(int col = 0, i = 0; i < line.length(); col = i) {
//                if(Character.isLetter(line.charAt(i))) {
//                    StringBuilder val = new StringBuilder();
//                    do { val.append(line.charAt(i)); }
...
//                        case "sidef":   tokens.accept(new Token(row, col, Token.TokenType.SIDEF));   break;
//                        case "System":
//                            if(!line.substring(i).startsWith("System.out.println")) error(row, col, line);
//                            tokens.accept(new Token(row, col, Token.TokenType.PRINTLN));
//                            i += 18;
//                            break;
//                        default: tokens.accept(new Token(row, col, Token.TokenType.ID, val.toString()));
//                    }
//                } else if(Character.isDigit(line.charAt(i))) {
//                    int val = 0;
//                    do { val = val*10 + Character.getNumericValue(line.charAt(i)); }
//                    while(++i < line.length() && Character.isDigit(line.charAt(i)));
//                } else switch(line.charAt(i)) {
//                    case ':':   tokens.accept(new Token(row, col, Token.TokenType.COLON));      break;
//                    case ';':   tokens.accept(new Token(row, col, Token.TokenType.SEMICOLON));  break;
//                    case '.':   tokens.accept(new Token(row, col, Token.TokenType.DOT));        break;
//                    case '!':   tokens.accept(new Token(row, col, Token.TokenType.BANG));       break;
...
//                        break;
//                    case '=':
//                        if(++i < line.length() && line.charAt(i) == '=') tokens.accept(new Token(row, col, Token.TokenType.EQUALS));
//                        else tokens.accept(new Token(row, col, Token.TokenType.EQSIGN));
//                        break;
//                    case '&':
//                        if(++i < line.length() && line.charAt(i) == '&') tokens.accept(new Token(row, col, Token.TokenType.AND));
//                        else error(row, col, line);
//                    case '|':
//                        if(++i < line.length() && line.charAt(i) == '|') tokens.accept(new Token(row, col, Token.TokenType.OR));
//                        else error(row, col, line);
//                    default: error(row, col, line);
//                }
//            }
//        return tokens.build();
//    }
